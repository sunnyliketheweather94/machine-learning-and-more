...
def last(values: Iterator[X]) -> X | None:
...
def converged(values: Iterator[X], done: Callable[[X, X], X]) -> X:
    ...
    Will loop forever if the input iterator doesn't end *or* converge.
    """
    result = last(converge(values=values, done=done))

    if result is None:
        raise ValueError("converged called on empty iterator")
...
def accumulate(
    ...
    *,
    ...
    return itertools.accumulate(iterable=iterable, func=func)
