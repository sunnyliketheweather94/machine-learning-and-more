...
class Distribution(ABC, Generic[A]):
    ...
    def map(self, f: Callable[[A], B]):  # returns a distribution of B
        return SampledDistribution(sampler=lambda: f(self.sample()))

    def apply(self, f: Callable):
        """f is a callable that takes an item A and returns a distribution of B"""
...
class FiniteDistribution(Distribution[A], ABC):
    ...
    def map(self, f: Callable[[A], B]):
        """returns a FiniteDistribution of type B"""
